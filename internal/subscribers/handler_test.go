...
func (m *MockService) Create(request *SubscriberRequest) (*string, error) {
...
func (m *MockService) Search(filter *SearchSubscribeRequest) (*SearchSubscribeResponse, error) {
	args := m.Called(filter)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*SearchSubscribeResponse), args.Error(1)
}
...
func TestHandler_Create_Success(t *testing.T) {
	...
	requestBody, err := json.Marshal(SubscriberRequest{})
...
func TestHandler_Search_Success(t *testing.T) {
	mockService := new(MockService)
	mockResponse := &SearchSubscribeResponse{}
